...
class Content_model extends CI_Model
{
	...
	public function get_recent_by_category($category_id, $limit = 10, $offset = 0, $date = 0, $content = 0)
	{
		...
		if($category_id > 0) {
			...
			if($category_id == 125) { // Berita Tasik
				$sql_where = " AND (`PC`.`category_id` = 125
								OR `PC`.`category_id` = 126
								OR `PC`.`category_id` = 127
								OR `PC`.`category_id` = 128
								OR `PC`.`category_id` = 129
								
								) ";
			} else if($category_id == 1) { // Umum
				$sql_where = " AND (`PC`.`category_id` = 1 
								OR `PC`.`category_id` = 3 
								OR `PC`.`category_id` = 4
								OR `PC`.`category_id` = 112) ";
