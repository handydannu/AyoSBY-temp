...
class Article_model extends CI_Model
{
	...
	public function get_recent_by_category($category_id, $limit = 10, $offset = 0, $date = 0, $content = 0)
	{
		if($category_id > 0) {
			...
			if($category_id == 1) { // Umum
				$sql_where = " AND (`PC`.`category_id` = 121
								OR `PC`.`category_id` = 125
								OR `PC`.`category_id` = 122
								OR `PC`.`category_id` = 124)";
				}
				else if($category_id ==8) { // News
		...
		} else if($category_id == 125) { // Semarang Raya
				$sql_where = " AND (`PC`.`category_id` = 125
								OR `PC`.`category_id` = 100
								OR `PC`.`category_id` = 101
								OR `PC`.`category_id` = 102
								OR `PC`.`category_id` = 103
								OR `PC`.`category_id` = 104
								OR `PC`.`category_id` = 105) ";
			} else if($category_id == 106) { // Wisata
				$sql_where = " AND (`PC`.`category_id` = 106
								OR `PC`.`category_id` = 107
								OR `PC`.`category_id` = 108
								OR `PC`.`category_id` = 109) ";

		//
// 			} else if($category_id == 60) { // Komunitas
// 				$sql_where = " AND (`PC`.`category_id` = 60
// 								OR `PC`.`category_id` = 70
// 								OR `PC`.`category_id` = 71) ";
...
// 								OR `PC`.`category_id` = 79) ";
	...
	function get_popular_monthly_by_category($category_id = 0, $limit = 10)
	{
		...
		if(count($arr_sql_where) > 0) $sql_where = " AND ".implode(' AND ', $arr_sql_where);

		$sql = "SELECT `P`.`post_id`, `P`.`post_date`, `P`.`post_subtitle`, `P`.`post_date_created`, `P`.`post_title`, `P`.`post_summary`, `P`.`post_image_thumb`, `P`.`post_image_content`, `P`.`post_type`, `P`.`post_feature`, `P`.`post_name`, P.`post_author`, `P`.`post_editor`, `PC`.`category_id`, `PH`.`post_hits` AS `hits`, `U`.`nama` AS `author_name`
				FROM `posts` `P`
					LEFT JOIN `posts_hits` `PH` ON `PH`.`post_id` = `P`.`post_id`
					LEFT JOIN `posts_category` `PC` ON `PC`.`post_id` = `P`.`post_id`
					LEFT JOIN `user` `U` ON (`U`.`uid` = `P`.`post_author`)
