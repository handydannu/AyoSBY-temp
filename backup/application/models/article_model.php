...
//			$this->db->or_where('posts_category.category_id', 21); // Internasional
				...
				$sql_where = " AND (`PC`.`category_id` = 1
								OR `PC`.`category_id` = 10
								OR `PC`.`category_id` = 11
...
// 								OR `PC`.`category_id` = 79) ";
	...
	function tag_populer() {
		$sql = "SELECT tag FROM `tag` ORDER BY `tag`.`count` DESC LIMIT 5";
