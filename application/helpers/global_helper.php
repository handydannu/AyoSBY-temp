...
function site_meta_dynamic()
{
	...
	$path_base 	= 'https://www.ayosurabaya.com/assets/img/';
	// $path_url 	= $path_base . 'Surabaya.png';
	$path_url 	= $path_base . 'ayosurabaya-logo.png?w=400';
	$logo_desc 	= '';
	...
	return array(
		'logo' => '<img src="' . $path_url . '" alt="Logo - ' . $logo_desc . 'AyoSurabaya.com" class="img-responsive" />',
		'desc' => $site_desc
	);
}
...
function metadata_seo()
{
	...
	switch ($CI->uri->segment(1)) {
		...
		case 'netizen':
			...
			$title 		= 'Halaman Netizen ' . ucrhypen($CI->uri->segment(1));
			$desc		.= $title;
			// $keyword 	= '';
			$canonical 	= set_tag_canonical(site_url($CI->uri->segment(1)));
			break;
		...
		case 'pedoman-media-siber':
			$title 		= 'Halaman Pedoman Pemberitaan Media Siber';
			$desc 		.= $title;
			// $keyword 	= '';
			$canonical 	= set_tag_canonical(site_url($CI->uri->segment(1)));
			break;
...
function meta_ogp()
{
	...
	switch ($CI->uri->segment(1)) {
		...
		case 'watch':
			...
			// $url = site_url('read') . '/' . $dc['year'] . '/' . $dc['month'] . '/' . $dc['day'] . '/' . $data['post']['post_id'] . '/' . /*$data['post']['category_id'] . '/' .*/ $data['post']['slug'];
			if ($CI->uri->segment(1) == 'watch') { // watch (video)
				$url_img = 'https://i.ytimg.com/vi/' . $data['post']['video'] . '/hqdefault.jpg';
			} else { // read & view
				$url_img = $CI->config->item('images_articles_uri') . $dc['year'] . '/' . $dc['month'] . '/' . $dc['day'] . '/' . $data['post']['post_id'] . '/' . $data['post']['post_image_content'];
			}
...
function get_alexa() // for ayosurabaya.com
{
	$CI = &get_instance();
	/** S: Alexa Certify Javascript*/
	echo "
	    <script type=\"text/javascript\">
	    _atrk_opts = { atrk_acct:\"XGeit1zDGU20kU\", domain:\"ayosurabaya.com\", dynamic: true};
	    (function() { var as = document.createElement('script'); as.type = 'text/javascript'; as.async = true; as.src = \"https://certify-js.alexametrics.com/atrk.js\"; var s = document.getElementsByTagName('script')[0];s.parentNode.insertBefore(as, s); })();
	    </script>
	    <noscript><img src=\"https://certify.alexametrics.com/atrk.gif?account=XGeit1zDGU20kU\" style=\"display:none\" height=\"1\" width=\"1\" alt=\"\" /></noscript>
